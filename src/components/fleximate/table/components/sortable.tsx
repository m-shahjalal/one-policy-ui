...
const SortableItem = React.forwardRef<HTMLDivElement, SortableItemProps>(
  (props, forwardedRef) => {
    ...
    // Add this custom utility function at the top of the file, after imports
    const customTransformUtils = {
      translateToString(
        transform: {
          x: number;
          y: number;
        ...
        const { x, y } = transform;
        // Only apply translation if x or y has a value
        if (x === 0 && y === 0) {
          return undefined;
        }
    ...
    const composedStyle = React.useMemo<React.CSSProperties>(() => {
      return {
        transform: customTransformUtils.translateToString(transform),
        transition,
        ...style,
      };
